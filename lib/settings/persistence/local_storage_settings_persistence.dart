...
class LocalStorageSettingsPersistence extends SettingsPersistence {
  ...
  Future<String> getCityName() async {
    final prefs = await instanceFuture;
    return prefs.getString('cityName') ?? 'Stavropol';
  }
  ...
  @override
  Future<bool> getNextLocation({required bool defaultValue}) async {
    final prefs = await instanceFuture;
    return prefs.getBool('nextLocation') ?? defaultValue;
  }

  @override
  Future<int> getCurrentYear() async {
    final prefs = await instanceFuture;
    return prefs.getInt('currentYear') ?? 1;
  }
  ...
  @override
  Future<void> saveCityName(String value) async {
    final prefs = await instanceFuture;
  ...
  @override
  Future<void> saveNextLocation(bool value) async {
    final prefs = await instanceFuture;
    await prefs.setBool('nextLocation', value);
  }

  @override
  Future<void> saveCurrentYear(int value) async {
    final prefs = await instanceFuture;
    await prefs.setInt('currentYear', value);
  }
}
